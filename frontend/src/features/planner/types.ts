...
export interface SchoolConfig { school_id: string; serving_line: string; meal_type: string; grade: string }
export interface SkylightConfig { email: string; frame_id: string }
export interface WeekResponse {
  week: DayMenu[]; kids: Kid[]; selections: Record<string, Record<number, SelectionState>>
  day_totals: Record<string, number>; day_sent: Record<string, number>; history: HistoryItem[]
...
export interface SendResult {
  ok: boolean; message: string; sent: number; deleted: number; skipped: number; errors: string[]
  results: PublicationKidResult[]; day_totals?: Record<string, number>; day_sent?: Record<string, number>; history?: HistoryItem[]
}
