"""Tests for the menu-item case formatting (school_menu.format_menu_item)."""
...
class TestTitleCaseFormatting:
    """SchoolCafe returns everything in ALL CAPS. The format_menu_item
    function normalizes this to Title Case while preserving acronyms."""

    def test_empty_string_passes_through(self):
        assert format_menu_item("") == ""

    def test_already_proper_case_passes_through(self):
        # Idempotent: don't reformat mixed-case text.
        assert format_menu_item("Cheese Pizza") == "Cheese Pizza"
    ...
    def test_preserves_bbq_acronym(self):
        ...
        assert format_menu_item("BRISKET BBQ SANDWICH") == "Brisket BBQ Sandwich"

    def test_preserves_pb_acronym(self):
        assert format_menu_item("PB AND J SANDWICH") == "PB And J Sandwich"

    def test_preserves_ampersand(self):
        assert format_menu_item("ROTINI & MEATBALLS") == "Rotini & Meatballs"
    ...
    def test_caches_repeated_calls(self):
        ...
        first = format_menu_item("CHEESE PIZZA")
        second = format_menu_item("CHEESE PIZZA")
    ...
    def test_common_items(self, source, expected):
        assert format_menu_item(source) == expected
